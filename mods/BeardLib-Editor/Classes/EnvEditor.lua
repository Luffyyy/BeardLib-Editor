...
function EnvEditor:build_default_menu()
    ...
    self:add_sky_param(self:PathItem("underlay", "scene", {text = "Underlay", group = skydome}, {"core/levels", "levels/zone"}))
    self:add_sky_param(self:PathItem("global_texture", "texture", {text = "Global cubemap", group = skydome}))
    self:add_sky_param(self:PathItem("global_world_overlay_texture", "texture", {text = "Global world overlay texture", group = global_textures}))
    self:add_sky_param(self:PathItem("global_world_overlay_mask_texture", "texture", {text = "Global world overlay mask texture", group = global_textures}))
    ...
    self:add_post_processors_param("shadow_processor", "shadow_rendering", "shadow_modifier", self:Slider("o1", nil, 1, {text = "Blend overlap(1st & 2nd)", offset = {0, 16}, min = 0, max = 10000, group = global_illumination}))
