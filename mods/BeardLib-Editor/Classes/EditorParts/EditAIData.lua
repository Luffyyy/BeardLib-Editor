...
function EditAIData:build_menu(parent)
	...
	self:TextBox("LocationId", callback(self._parent, self._parent, "set_unit_data"), "", {help = "Select a location id to be associated with this navigation point"})
	self:Divider("LocOfLocation", {text = "Text = " .. managers.localization:text("location_unknown")})
	self:NumberBox("SuspicionMultiplier", callback(self._parent, self._parent, "set_unit_data"), 1, {min = 1, floats = 1, help = "multiplier applied to suspicion buildup rate"})
	self:NumberBox("DetectionMultiplier", callback(self._parent, self._parent, "set_unit_data"), 1, {min = 0.01, help = "multiplier applied to AI detection speed. min is 0.01"})
	return sizer
end
